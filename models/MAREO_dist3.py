...
class Model(nn.Module):
	def __init__(self, task_gen, args):
		...
		self.encoder = Enc_Conv_Mq16_dist3fc() 
		...
		self.y_out = nn.Linear(self.key_size * task_gen.y_dim, task_gen.y_dim)
		self.ydim = task_gen.y_dim
		...
		# seq length depending on the task:
		self.seq_len = 4 # args.seq_len
	...
	def inner_loop(self, x_seq, device, m):
		...
		x_in1 = torch.cat((x_seq[:,0,:,:], x_seq[:,1,:,:], x_seq[:,2,:,:]), 1).unsqueeze(1)
		x_in2 = torch.cat((x_seq[:,3,:,:], x_seq[:,4,:,:], x_seq[:,5+m,:,:]), 1).unsqueeze(1)
